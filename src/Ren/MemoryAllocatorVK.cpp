...
Ren::MemAllocation Ren::MemoryAllocator::Allocate(const uint32_t size, const char *tag) {
    while (true) {
        for (uint32_t i = 0; i < uint32_t(blocks_.size()); ++i) {
            ...
            const uint32_t alloc_off = blocks_[i].alloc.Alloc_Recursive(0, size, tag);
...
void Ren::MemoryAllocator::Free(uint32_t block_ndx, uint32_t alloc_off) {
    ...
    const int node_ndx = blocks_[block_ndx].alloc.Find_Recursive(0, alloc_off);
    assert(node_ndx != -1);
