...
using program_load_callback =
    std::function<ProgramRef(const char *name, const char *arg1, const char *arg2, const char *arg3, const char *arg4)>;
...
class Material : public RefCounter {
  ...
  public:
    ...
    Material(const char *name, const char *mat_src, eMatLoadStatus *status, const program_load_callback &on_prog_load,
             const texture_load_callback &on_tex_load, const sampler_load_callback &on_sampler_load, ILog *log);
    Material(const char *name, uint32_t flags, const ProgramRef programs[], int programs_count,
    ...
    void Init(uint32_t flags, const ProgramRef _programs[], int programs_count, const Tex2DRef _textures[],
              const SamplerRef _samplers[], int textures_count, const Vec4f _params[], int params_count, ILog *log);
    void Init(const char *mat_src, eMatLoadStatus *status, const program_load_callback &on_prog_load,
              const texture_load_callback &on_tex_load, const sampler_load_callback &on_sampler_load, ILog *log);
};
