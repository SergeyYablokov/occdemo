...
void RpUpdateBuffers::Execute(RpBuilder &builder) {
    ...
    if (skin_transforms_.count) {
        ...
        void *pinned_mem = glMapBufferRange(GL_SHADER_STORAGE_BUFFER, orphan_index_ * SkinTransformsBufChunkSize,
    ...
    if (shape_keys_.count) {
        ...
        void *pinned_mem = glMapBufferRange(GL_SHADER_STORAGE_BUFFER, orphan_index_ * ShapeKeysBufChunkSize,
    ...
    if (!instances_buf.tbos[orphan_index_]) {
        ...
        assert((offset % ctx.capabilities.tex_buf_offset_alignment == 0) && "Offset is not properly aligned!");

        char name_buf[32];
        sprintf(name_buf, "Instances TBO #%i", orphan_index_);

        instances_buf.tbos[orphan_index_] = ctx.CreateTexture1D(
            name_buf, instances_buf.ref, Ren::eTexFormat::RawRGBA32F, offset, InstanceDataBufChunkSize);
    }
    ...
    if (instances_.count) {
        ...
        void *pinned_mem = glMapBufferRange(GL_TEXTURE_BUFFER, orphan_index_ * InstanceDataBufChunkSize,
    ...
    if (!cells_buf.tbos[orphan_index_]) {
        ...
        cells_buf.tbos[orphan_index_] =
            ctx.CreateTexture1D(name_buf, cells_buf.ref, Ren::eTexFormat::RawRG32U, offset, CellsBufChunkSize);
    }
    ...
    if (cells_.count) {
        ...
        void *pinned_mem = glMapBufferRange(GL_TEXTURE_BUFFER, orphan_index_ * CellsBufChunkSize, CellsBufChunkSize,
                                            BufferRangeMapFlags);
        if (pinned_mem) {
    ...
    if (!lights_buf.tbos[orphan_index_]) { // Create buffer for lights information
        ...
        lights_buf.tbos[orphan_index_] =
            ctx.CreateTexture1D(name_buf, lights_buf.ref, Ren::eTexFormat::RawRGBA32F, offset, LightsBufChunkSize);
    }
    ...
    if (light_sources_.count) {
        ...
        void *pinned_mem = glMapBufferRange(GL_TEXTURE_BUFFER, orphan_index_ * LightsBufChunkSize, LightsBufChunkSize,
                                            BufferRangeMapFlags);
        if (pinned_mem) {
    ...
    if (!decals_buf.tbos[orphan_index_]) {
        ...
        decals_buf.tbos[orphan_index_] =
            ctx.CreateTexture1D(name_buf, decals_buf.ref, Ren::eTexFormat::RawRGBA32F, offset, DecalsBufChunkSize);
    }
    ...
    if (decals_.count) {
        ...
        void *pinned_mem = glMapBufferRange(GL_TEXTURE_BUFFER, orphan_index_ * DecalsBufChunkSize, DecalsBufChunkSize,
                                            BufferRangeMapFlags);
        if (pinned_mem) {
    ...
    if (!items_buf.tbos[orphan_index_]) {
        ...
        items_buf.tbos[orphan_index_] =
            ctx.CreateTexture1D(name_buf, items_buf.ref, Ren::eTexFormat::RawRG32U, offset, ItemsBufChunkSize);
    }
    ...
    if (items_.count) {
        ...
        void *pinned_mem = glMapBufferRange(GL_TEXTURE_BUFFER, orphan_index_ * ItemsBufChunkSize, ItemsBufChunkSize,
                                            BufferRangeMapFlags);
        if (pinned_mem) {
    ...
    { // Prepare data that is shared for all instances
        ...
        void *pinned_mem = glMapBufferRange(GL_UNIFORM_BUFFER, orphan_index_ * SharedDataBlockSize,
