...
void RpDownColor::Setup(RpBuilder &builder, const ViewState *view_state, int orphan_index, const char shared_data_buf[],
...
void RpDownColor::Execute(RpBuilder &builder) {
    ...
    bindings[1] = {Ren::eBindTarget::UBuf, REN_UB_SHARED_DATA_LOC, orphan_index_ * SharedDataBlockSize,
...
void RpDownColor::LazyInit(Ren::Context &ctx, ShaderLoader &sh) {
    if (!initialized) {
        blit_down_prog_ = sh.LoadProgram(ctx, "blit_down", "internal/blit.vert.glsl", "internal/blit_down.frag.glsl");
