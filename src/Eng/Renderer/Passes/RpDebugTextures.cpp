...
void RpDebugTextures::Execute(RpBuilder &builder) {
    ...
    if (((render_flags_ & debug_cull) == debug_cull) && depth_pixels_) {
        ...
        x_offset += BlitTex(applied_state, x_offset, 0, depth_w_ / 2, depth_h_ / 2, temp_tex_, 1.0f);
    }
    ...
    if (render_flags_ & DebugBVH) {
        ...
        nodes_buf_->FreeRegion(0);
        const uint32_t off = nodes_buf_->AllocRegion(buf_size, "nodes debug", temp_stage_buf.get());
        assert(off == 0);
...
void RpDebugTextures::LazyInit(Ren::Context &ctx, ShaderLoader &sh) {
    ...
    { // setup temp vao
        const Ren::VtxAttribDesc attribs[] = {{vtx_buf1->handle(), REN_VTX_POS_LOC, 2, Ren::eType::Float32, 0,
    ...
    if (!output_fb_.Setup(ctx.api_ctx(), nullptr, ctx.w(), ctx.h(), &output_tex_, 1, {}, {}, false)) {
