...
void RpDOF::Setup(RpBuilder &builder, const Ren::Camera *draw_cam, const ViewState *view_state,
    ...
    }
    {
        ...
        blur_temp_4x_[0] = builder.WriteTexture("DOF temp 1", params, *this);
        blur_temp_4x_[1] = builder.WriteTexture("DOF temp 2", params, *this);
    }
...
void RpDOF::Execute(RpBuilder &builder) {
    ...
    { // blur coc buffer
        PrimDraw::Binding binding = {Ren::eBindTarget::Tex2D, REN_BASE0_TEX_SLOT, coc1_tex.ref->handle()};
    ...
    { // calc near coc
        ...
        const PrimDraw::Binding bindings[2] = {{Ren::eBindTarget::Tex2D, REN_BASE0_TEX_SLOT, coc1_tex.ref->handle()},
    ...
    { // apply 3x3 blur to coc
        const PrimDraw::Binding binding = {Ren::eBindTarget::Tex2D, REN_BASE0_TEX_SLOT, blur2_4x_tex.ref->handle()};
    ...
    { // blur color buffer
        ...
        PrimDraw::Binding bindings[2] = {{Ren::eBindTarget::Tex2D, REN_BASE0_TEX_SLOT, down_depth_4x_tex.ref->handle()},
    ...
    { // combine dof buffers, apply blur
        ...
        bindings[3] = {Ren::eBindTarget::Tex2D, REN_BASE1_TEX_SLOT, blur1_4x_tex.ref->handle()};
...
void RpDOF::LazyInit(Ren::Context &ctx, ShaderLoader &sh, RpAllocTex &down_depth_4x_tex, RpAllocTex &blur1_temp_4x,
    ...
    if (!coc_fb_[0].Setup(ctx.api_ctx(), nullptr, coc1_tex.desc.w, coc1_tex.desc.h, coc1_tex.ref, {}, {}, false)) {
        ctx.log()->Error("RpDOF: coc_fb_[0] init failed!");
    }

    if (!coc_fb_[1].Setup(ctx.api_ctx(), nullptr, coc2_tex.desc.w, coc2_tex.desc.h, coc2_tex.ref, {}, {}, false)) {
        ctx.log()->Error("RpDOF: coc_fb_[1] init failed!");
    }

    if (!blur_fb_[0].Setup(ctx.api_ctx(), nullptr, blur1_temp_4x.desc.w, blur1_temp_4x.desc.h, blur1_temp_4x.ref, {},
                           {}, false)) {
        ctx.log()->Error("RpDOF: blur_fb_[0] init failed!");
    }

    if (!blur_fb_[1].Setup(ctx.api_ctx(), nullptr, blur2_temp_4x.desc.w, blur2_temp_4x.desc.h, blur2_temp_4x.ref, {},
                           {}, false)) {
    ...
    if (!dof_fb_.Setup(ctx.api_ctx(), nullptr, output_tex.desc.w, output_tex.desc.h, output_tex.ref, {}, {}, false)) {
