...
void Renderer::ExecuteDrawList(const DrawList &list, const PersistentBuffers &persistent_bufs, const FrameBuf *target) {
    using namespace RendererInternal;
    ...
    { // Setup render passes
        ...
        //
        if (list.env.env_map) {
            rp_skydome_.Setup(rp_builder_, list, &view_state_, SHARED_DATA_BUF, MAIN_COLOR_TEX, MAIN_SPEC_TEX,
                              MAIN_DEPTH_TEX);
        ...
        if ((list.render_flags & (EnableZFill | DebugWireframe)) == EnableZFill) {
            rp_depth_fill_.Setup(rp_builder_, list, &view_state_, &persistent_bufs, INSTANCES_BUF, SHARED_DATA_BUF,
