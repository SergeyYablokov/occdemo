...
Gui::Renderer::Renderer(Ren::Context &ctx, const JsObject &config) : ctx_(ctx) {
    ...
    vertex_buf_ =
        ctx_.CreateBuffer(name_buf, Ren::eBufType::VertexAttribs,
                          Ren::eBufAccessType::Draw, Ren::eBufAccessFreq::Dynamic,
                          FrameSyncWindow * MaxVerticesPerRange * sizeof(vertex_t));

    sprintf(name_buf, "UI_IndexBuffer [%i]", instance_index);

    index_buf_ =
        ctx_.CreateBuffer(name_buf, Ren::eBufType::VertexIndices,
                          Ren::eBufAccessType::Draw, Ren::eBufAccessFreq::Dynamic,
